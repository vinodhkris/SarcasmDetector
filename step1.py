...
file1 = open("positivewords.txt","w")
file1.write("")
file1.close()
file1 = open("negativewords.txt","w")
file1.write("")
file1.close()
file1 = open("positivepredicatewords.txt","w")
file1.write("")
file1.close()
#Prepare the files
...
def extractngramspos(inputFile,seed_word):
	...
	for line in inputFile:
		...
		poslist = extractWordPos(seed_word,line.strip(),1)
		...
		for pos in poslist:
			if pos!=-1 and line[pos]!=seed_word:
				...
				try:
					...
					asdf = 0
		num +=1
	#print "pos words",[unigram,bigram,trigram]
...
def extractngramsneg(inputFile,seed_word):
	...
	numword = 0
	for line in inputFile:
		poslist = extractWordPos(seed_word,line.strip(),0)
		#print "seed_word",seed_word,"line",line,"poslist",poslist
		for pos in poslist:
			if pos!=-1 and line[pos]!=seed_word:
				if posTags[num][pos] == "V":
					unigram.append(line.split()[pos])
...
def calculateScorepos(poswords, phrase, posFile, negFile):
	...
	for line in posFile:
		for word in poswordslist:
			poslist = extractWordPos(word,line.strip(),1)
			for pos in poslist:
				if pos!=-1:					
					#print "1111", line, "\n", word, phrase									#To check if the positive word is in the line and put the pointer after that
					try:
					#	print line.split()[pos:pos+numofgrams]
	...
	if numofgrams < 3:
		if (countpos+countneg) <4:
			return 0
	#print countpos, countneg, phrase
...
def calculateScoreneg(negwords, phrase, posFile, negFile):
	...
	for line in posFile:
		for word in negwordslist:								#Added to check if the positive word occurs before a negative phrase not THE negative phrase
			poslist = extractWordPos(word,line.strip(),0)
			for pos in poslist:
...
def extractPredicates(negative_seed_word, inputFile)	:
	...
	for line in inputFile:
		for word in copularVerbs:
			poslist = extractWordPosPredicate(word.strip(),line.strip(),1)			#added inflections here; pointer points to the location right after the copular verb
			for pos in poslist:
				if pos!=-1:														#Copular verb found
					pos1list = extractWordPos(negative_seed_word,line.strip(),0)		#see if the negative word is there
...
def calculateScorePredicates(negwords, phrase, posFile, negFile):
	...
	for line in posFile:
		for word in negwordslist:
			poslist = extractWordPos(word,line.strip(),0)
			pos1list = extractWordPos(phrase,line.strip(),0)
			for pos in poslist:
				for pos1 in pos1list:
					if pos!=-1 and pos1!=-1:
						pos+=1
	...
	for line in negFile:
		for word in negwordslist:							#for all words in the negative word list
			poslist = extractWordPos(word,line.strip(),0)				#FInd if the negative word is there in the sentence
			pos1list = extractWordPos(phrase,line.strip(),0)		#Find if the positive predicate is in the sentence
			for pos in poslist:
				for pos1 in pos1list:
					if pos!=-1 and pos1!=-1:
...
while len(positivewords - prevpositivewords) > 0:
	...
	for numofwords in xrange(len(sorted_score)):
		...
		#	for words1 in negativewords:
		#		if sorted_score[numofwords][0] in words1:
		#			print "negative words before", negativewords
		#			negativewords = negativewords.difference({words1})	
		#			print "word subtracted",words1,"word compared to",sorted_score[numofwords][0]
		#			print "negatvie words after",negativewords
		#			break
	...
	for numofwords in xrange(len(sorted_score)):
		if countnotsubwords <=10 and sorted_score[numofwords][1] > 0.5:									#Taking the top 20
			if not subsumed(sorted_score[numofwords][0],positivewords):				#Check if the words are subsumed already in the negative words class
				#for words1 in positivewords:
				#	if sorted_score[numofwords][0] in words1:
				#		print words1, sorted_score[numofwords][0]
				#		positivewords = positivewords.difference({words1})	
				#		break
				countnotsubwords+=1
